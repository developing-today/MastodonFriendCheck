...
function getCallbackTimestamp (group, callback) {
  ...
  object[timestamp].callback = callback
    ? callback
    : function (result) {
        log('defaultCallback_getCallbackTimestamp', 'got result', { result })
      }
  log({
    group,
    timestamp,
      ...
      sendMessage(
        ...
        getCallbackTimestamp('follow', function (result) {
          ...
          if (
            ...
            log('addFollowListener', 'got unexpected result', result)
          }
        })
      )
    },
...
async function handleMessage (result) {
  ...
  if (type && url) {
    if (type === 'open') {
      ...
      log('handleMessage', 'not following', 'addFollowListener', url)
